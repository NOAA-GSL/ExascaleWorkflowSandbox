...
class Experiment:
    ...
    def _make_truth_config(self):
        ...
        merge_config_str(truth_config, textwrap.dedent(f"""
    ...
    def _make_obs_config(self, t):
        ...
        ny = self.config["truth"]["ny"]
        dt = self.config["truth"]["tstep"]
        window_begin = t + timedelta(0, leadtime.fcst_to_seconds(self.config["assimilation"]["window"]["begin"]))
        window_begin_str = window_begin.strftime("%Y-%m-%dT%H:%M:%SZ")
        window_length = self.config["assimilation"]["window"]["length"]
        truth_leadtime = leadtime.seconds_to_fcst(int((window_begin - exp_begin).total_seconds()))
        truth_path = f"{self.config['experiment']['path']}/truth"
        ...
        merge_config_str(make_obs_config, textwrap.dedent(f"""
        geometry:
          nx: {nx}
          ny: {ny}
        initial condition:
    ...
    def _make_3dvar_config(self, t):
        ...
        merge_config_str(var3d_config, textwrap.dedent(f"""
        cost function:
          cost type: 3D-Var
          time window:
            begin: "{window_begin_str}"
